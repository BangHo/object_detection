...
class ObjectDetection(Node):
    def __init__(self, img_size: Union[Tuple[int, int], None] = (640, 640)
        ...
        self.video_capture_right = cv2.VideoCapture("/dev/v4l/by-id/usb-046d_罗技高清网络摄像机_C930c_4366BDCE-video-index0", cv2.CAP_V4L2)
        self.video_capture_right.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        self.video_capture_right.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)

        # Timer callback
        self.frequency = 70   # Hz : 1초에 20번 타이머 콜백 실행
        ...
        self.image_publisher = self.create_publisher(Image, "/video1", 1)
    ...
    def timer_callback(self):
        ...
        if ret_right:
            ...
            self.image = frame_right.copy()
            self.YOLOv7_detect()

            self.image_msg = self.bridge.cv2_to_imgmsg(self.image, "bgr8")
